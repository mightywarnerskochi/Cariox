...
<div class="dashboard-content">
    ...
    <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 1.5rem;">
        <h2 style="font-size: 1.5rem; font-weight: 600; color: #1e293b; margin: 0;">Clients Management</h2>
    </div>
    ...
    <div class="section-container">
        ...
        <div class="section-body">
            <form action="{{ route('admin.client.storeSection') }}" method="POST" class="section-form" novalidate>
                @csrf
                ...
                <div class="form-group">
                    ...
                    <textarea name="description" class="form-control tinymce" rows="3" required>{{ old('description', $sectionContent->description) }}</textarea>
    ...
    <!-- Clients Table -->
    <div class="section-container">
        <form action="{{ route('admin.client.bulkDelete') }}" method="POST" id="clientBulkForm">
            @csrf
            @method('DELETE')
            <div class="section-header" style="background-color: transparent;">
                <div style="display: flex; gap: 1rem; align-items: center;">
                    <button type="button" onclick="submitBulkDelete('clientBulkForm', '{{ route('admin.client.bulkDelete') }}')" class="btn-update" style="color: white; background: #ef4444; border-color: #ef4444;">Delete Selected</button>
                    <button type="button" onclick="submitBulkToggle('clientBulkForm', '{{ route('admin.client.bulkToggleStatus') }}')" class="btn-update" style="color: white; background: #64748b; border-color: #64748b;">Toggle Status</button>
                    <span style="font-size: 0.875rem; color: #64748b; font-weight: 400;">Manage Clients. Active clients appear on the frontend.</span>
                </div>
                <div>
                    <a href="{{ route('admin.client.create') }}" class="btn-add">Add Client</a>
                </div>
            </div>
            <div>
                    ...
                    <tbody>
                        @forelse($clients as $index => $client)
                            <tr>
                                ...
                                <td>{{ $index + 1 }}</td>
                                <td>
                                    <img src="{{ Storage::url($client->image) }}" alt="{{ $client->alt_text }}" style="height: 40px; border-radius: 4px;">
                                </td>
                                <td>{{ $client->name }}</td>
                                <td>{{ $client->alt_text ?: '-' }}</td>
                                <td>
                                    <select class="form-control" style="padding: 0.25rem;" onchange="updatePosition('{{ route('admin.client.update', $client->id) }}', this.value)">
                                ...
                                <td>
                                    <label class="toggle-switch">
                                        <input type="checkbox" onchange="toggleStatus('{{ route('admin.client.toggleStatus') }}', {{ $client->id }})" {{ $client->status ? 'checked' : '' }}>
                                        <span class="slider"></span>
                                    </label>
                                </td>
                                <td style="display:flex;">
                                    <a href="{{ route('admin.client.edit', $client->id) }}" class="action-btn" style="text-decoration:none;"><i class="fas fa-pencil-alt"></i></a>
                                    <button type="button" onclick="deleteItem('{{ route('admin.client.destroy', $client->id) }}')" class="action-btn" style="color: #ef4444;"><i class="fas fa-trash"></i></button>
                                </td>
                            </tr>
                        @empty
                            <tr>
                                <td colspan="8" style="text-align: center; padding: 2rem; color: #64748b;">No clients added yet.</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
