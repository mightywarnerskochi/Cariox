...
@section('content')
<?php
$page_title = 'Blogs | Cariox';
$page_desc = '';

?>
<div class="inner-vector">
    <picture><img src="{{ asset('assets/images/inner/inner-vector.png') }}" alt="Blogs"></picture>
</div>
...
<section class="blog-listing-page commonPadding-120">
    <div class="container-ctn">
        <div class="top-featured-blogs">
            @if($blogs->count() > 0)
            @php 
                $firstBlog = $blogs->shift(); 
                $sidebarBlogs = $blogs->splice(0, 4); 
            @endphp
            <div class="featured-main-blog">
                <div class="img-wrapper">
                    <img src="{{ asset('storage/' . $firstBlog->image) }}" alt="{{ $firstBlog->title }}" width="1056" height="470" class="img-fluid">
                </div>
                <div class="content">
                    <h2>{{ $firstBlog->title }}</h2>
                    <p>{{ Str::limit(strip_tags($firstBlog->detailed_description), 200) }}</p>
                </div>
                <div class="action-footer">
                    <div class="meta-info">
                        <div class="meta-item">
                            <img src="{{ asset('assets/images/testimonial/1.png') }}" alt="Admin" class="img-fluid rounded-circle" >
                            <span>Post By : Admin</span>
                        </div>
                        <div class="meta-item border-left-divider">
                            <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg"><rect x="4" y="5" width="16" height="15" rx="2" stroke="#4A5568" stroke-width="1.5"/><path d="M16 3V7M8 3V7M4 10H20" stroke="#4A5568" stroke-width="1.5" stroke-linecap="round"/></svg>
                            <span>{{ $firstBlog->created_at->format('d M, Y') }}</span>
                        </div>
                    </div>
                    <div class="btn-wrap">
                        <a href="{{ route('blog-detail', $firstBlog->slug) }}" class="btn btn-gradient">Learn More</a>
                    </div>
                </div>
            </div>

            <div class="featured-sidebar-blogs">
                @foreach($sidebarBlogs as $sideBlog)
                <div class="list-blog-item">
                    <div class="img-wrapper">
                        <img src="{{ asset('storage/' . $sideBlog->image) }}" alt="{{ $sideBlog->title }}" width="152" height="183" class="img-fluid">
                    </div>
                    <div class="content">
                        <h5>{{ $sideBlog->title }}</h5>
                        <div class="meta-info">
                            <div class="meta-item">
                                <img src="{{ asset('assets/images/testimonial/1.png') }}" alt="Admin" class="img-fluid rounded-circle">
                                <span>Post By: {{$sideBlog->author}}</span>
                            </div>
                            <div class="meta-item border-left-divider">
                                ...
                                <span>{{ $sideBlog->created_at->format('d M, Y') }}</span>
                            </div>
                        </div>
                        <a href="{{ route('blog-detail', $sideBlog->slug) }}" class="read-more-link">Read More</a>
                    </div>
                </div>
                @endforeach
            </div>
        ...
        <div class="blog-grid-section">
            <div class="blog-grid">
                ...
                <div class="grid-blog-item">
                    <div class="blog-card">
                        <div class="img-wrapper">
                            <img src="{{ asset('storage/' . $blog->image) }}" width="359" height="270" alt="{{ $blog->title }}" class="img-fluid">
                        </div>
                        <div class="content">
                    ...
                    <div class="button-wrapper">
                        <a href="{{ route('blog-detail', $blog->slug) }}" class="read-more-link">Read More</a>
                    </div>
                </div>
                @endforeach
            </div>
            ...
            <div class="pagination-wrapper mt-5">{{ $blogs->links() }}</div>
        </div>
    </div>
</section>
@endsection