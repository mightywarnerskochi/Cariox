...
<section class="services-page-main commonPadding-120">
    <div class="container-ctn">
        <div class="services-header d-flex flex-wrap justify-content-between align-items-start mb-5">
            ...
            {!! $serviceSection->description ?? '<p>Default description</p>' !!}
            </div>
        </div>
                ...
                <div class="grid-left-col">
                    @foreach($services as $index => $service)
                        ...
                        @php
                            ...
                            if (str_contains($nameLower, 'supply')) {
                                $cardClass = 'card-supply';
                            } elseif (str_contains($nameLower, 'install')) {
                                $cardClass = 'card-installation';
                            } elseif (str_contains($nameLower, 'maintenance') || str_contains($nameLower, 'amc')) {
                                $cardClass = 'card-amc';
                            } else {
                                // Fallback to reference order (01, 02, 03)
                                $cardClass = $index === 0 ? 'card-supply' : ($index === 1 ? 'card-installation' : 'card-amc');
                            }

                            if ($cardClass === 'card-supply') {
                                $fallbackImg = asset('assets/images/home/services/1.png');
                            } elseif ($cardClass === 'card-installation') {
                                $fallbackImg = asset('assets/images/home/services/2.png');
                            } else {
                                $fallbackImg = asset('assets/images/home/services/3.png');
                            }
                        @endphp

                        <div class="service-detail-card {{ $cardClass }}">
                            <div class="text-content">
                                <h3>{{ $service->name }}</h3>
                                <p>{{ strip_tags($service->home_description) }}</p>
                            </div>
                            <span class="card-num">{{ str_pad($index + 1, 2, '0', STR_PAD_LEFT) }}</span>
                            <div class="card-image">
                                <img src="{{ $service->main_image ? asset('storage/' . $service->main_image) : $fallbackImg }}" alt="{{ $service->name }}">
                            </div>
                            <a href="{{ route('service-detail', $service->slug) }}" class="arrow-icon">
